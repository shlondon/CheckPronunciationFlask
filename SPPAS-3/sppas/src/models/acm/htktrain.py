...
from sppas.src.annotations.annotationsexc import NoTierInputError

import sppas.src.anndata as anndata
from sppas.src.anndata import sppasTranscription, sppasTrsRW, sppasLabel, sppasTag
...
def test_command(command):
    ...
    """
    return Process().test_command(command)
    # try:
...
class sppasTrainingCorpus(object):
    ...
    def add_file(self, trs_filename, audio_filename):
        ...
        try:
            parser = sppasTrsRW(trs_filename)
    ...
    def _add_tier(self, tier, outfile, ext):
        try:
            ...
            out_filename = os.path.join(self.datatrainer.get_storetrs(),
                                        outfile+ext)
            parser = sppasTrsRW(out_filename)
            parser.write(trs)
        except:
            return False
        return True
...
class sppasHTKModelTrainer(object):
    ...
    def align_trs(self):
        ...
        for trs_filename, trs_workfile in self.corpus.transfiles.items():
            ...
            parser = sppasTrsRW(trs_workfile)
