# -*- coding:utf-8 -*-
"""
:filename: sppas.src.analysis.tests.test_filters.py
:author:   Brigitte Bigi
:contact:  develop@sppas.org
:summary:  Tests of the filter system.
...
class TestFiltersTier(unittest.TestCase):
    ...
    def test_nlab(self):
        ...
        """
        parser = sppasTrsRW(os.path.join(DATA, "F_F_B003-P8-token.xra"))
        trs = parser.read(heuristic=False)
        tier = trs.find("Tokens")  # 5 ipus, 5 silences, 1 empty annotation
        f = sppasTierFilters(tier)
...
class TestRelationFilterTier(unittest.TestCase):
    ...
    def setUp(self):
        parser = sppasTrsRW(os.path.join(DATA, "relfit.xra"))
        self.trs = parser.read(heuristic=False)
