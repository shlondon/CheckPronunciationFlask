...
"""
    ...
    ui.phoenix.page_analyze.datalist.mediaslist.py
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    A class to display a summary of a list of sppasMedia (see anndata).
...
class MediaListCtrl(BaseObjectListCtrl):
    ...
    def _create_columns(self):
        """Create a listctrl to display the media links."""
        self.AppendColumn("type", format=wx.LIST_FORMAT_LEFT, width=sppasPanel.fix_size(40))
        self.AppendColumn("filename", format=wx.LIST_FORMAT_LEFT, width=sppasPanel.fix_size(300))
        self.AppendColumn("mimeheader", format=wx.LIST_FORMAT_LEFT, width=sppasPanel.fix_size(80))
    ...
    def update_item(self, obj):
        ...
        if obj.get_id() in self._trss:
            ...
            self.SetItem(index, 0, "Media")
            self.SetItem(index, 1, obj.get_filename())
            self.SetItem(index, 2, obj.get_mime_type())
