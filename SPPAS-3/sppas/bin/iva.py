...
# -*- coding: UTF-8 -*-
"""
:filename: sppas.bin.iva.py
:author:   Brigitte Bigi
:contact:  develop@sppas.org
:summary:  Run the IVA automatic annotation.
...
from sppas.src.annotations import sppasIVA
...
if __name__ == "__main__":
    ...
    parameters = sppasParam(["iva.json"])
    ann_step_idx = parameters.activate_annotation("iva")
    ...
    group_io_1.add_argument(
        "-i",
        metavar="file",
        help='An input file with values and/or segments.')
    ...
    arguments = vars(args)
    for a in arguments:
        if a not in ('i', 's', 'o', 'I', 'e', 'quiet', 'log'):
    ...
    if args.i or args.s:
        ...
        ann = sppasIVA(log=None)
        ann.fix_options(parameters.get_options(ann_step_idx))
        input_files = None
        if args.i:
            if args.s:
                input_files = [args.i, args.s]
            else:
                input_files = [args.i]
        else:
            input_files = [args.s]
        ...
        if args.o:
            ann.run(input_files, output=args.o)

        else:
            trs = ann.run(input_files)
            for tier in trs:
                print(tier.get_name())
                for a in tier:
                    print("{} {} {:s}".format(
